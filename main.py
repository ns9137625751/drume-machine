...
from transformers import LineByLineTextDataset, RagConfig
pygame.init()
...
def draw_load_menu(index):
    ...
    for beat in range(len(saved_beats)):
        ...
        if 0<=index<len(saved_beats) and beat==index:
            ...
            loaded_bpm=int(saved_beats.beat[beat][beats_index_end+6:bpm_index_end])
...
while run:
    ...
    beats_text=medium_font.render('Beats in Loos',True,white)
